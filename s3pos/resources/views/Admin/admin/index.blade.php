...
@section('content')
    ...
    <div class="alert alert-danger mt-3" role="alert">
        <i class="fas fa-exclamation-triangle"></i> Chỉ được phép xóa những quản trị viên đang bị khóa!
    </div>
    @include('Admin.admin.create')
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('admin.admin.update') }}" id="form-update" method="POST" enctype="multipart/form-data">
                @csrf
...
@push('js')
    <script>
        const routeList = "{{ route('admin.admin.list') }}";
        const routeUpdate = "{{ route('admin.admin.update') }}";
        filterTable(page = 1);
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('admin.admin.delete') }}");
        }
