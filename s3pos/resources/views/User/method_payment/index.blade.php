...
@section('content')
    <div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
        ...
        <div class="content flex-row-fluid" id="kt_content">
            <div class="d-flex justify-content-between py-5">
                <div class="">
                    <h1 class="d-flex text-white fw-bold my-1 fs-3">Phương thức thanh toán</h1>
                ...
                @can('method_payment-create')
                    <button class="btn btn-primary h-40px btn-add">
                        Tạo mới
                    </button>
                    @include('User.method_payment.modal_add')
                @endcan
            </div>
            <!--begin::Products-->
    ...
    <div class="modal fade" id="modal-edit" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
        <form action="{{ route('method_payment.update') }}" id="form-update" method="POST" enctype="multipart/form-data">
...
@section('script')
    <script>
        const routeList = "{{ route('method_payment.list') }}";
        const routeUpdate = "{{ route('method_payment.update') }}";
        filterTable();
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('method_payment.delete') }}");
        }

        function changeStatus(id) {
            $.post("{{ route('method_payment.update') }}", {
                id
            }, function(rs) {
                Toast.fire({
                    icon: rs?.type,
