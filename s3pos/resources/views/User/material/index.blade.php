...
@section('content')
    <div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
        ...
        <div class="content flex-row-fluid" id="kt_content">
            <div class="d-flex justify-content-between py-5">
                <div class="">
                    <h1 class="d-flex text-white fw-bold my-1 fs-3">Quản lý nguyên liệu</h1>
                    ...
                    <table class="table align-middle table-bordered fs-6 gy-5">
                        <thead class="bg-primary">
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                <th class="w-150px text-center">Mã</th>
                                <th class="">Tên</th>
    ...
    @include('User.material.modal_add')
    <div class="modal fade" id="modal-edit" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
        <form action="{{ route('material.update') }}" id="form-update" method="POST" enctype="multipart/form-data">
...
@section('script')
    <script>
        const routeList = "{{ route('material.list') }}";
        const routeUpdate = "{{ route('material.update') }}";
        filterTable();
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('material.delete') }}");
        }

        function changeStatus(id) {
            $.post("{{ route('material.update') }}", {
                id
            }, function(rs) {
                Toast.fire({
                    icon: rs?.type,
