...
@section('content')
    <div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
        ...
        <div class="content flex-row-fluid" id="kt_content">
            <div class="d-flex justify-content-between py-5">
                <div class="">
                    <h1 class="d-flex text-white fw-bold my-1 fs-3">Nguồn bán hàng</h1>
                ...
                @can('sale_source-create')
                    <button class="btn btn-primary h-40px btn-add">
                        Tạo mới
                    </button>
                    @include('User.sale_source.modal_add')
                @endcan
            </div>
            <!--begin::Products-->
    ...
    <div class="modal fade" id="modal-edit" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
        <form action="{{ route('sale_source.update') }}" id="form-update" method="POST" enctype="multipart/form-data">
            <div class="modal-dialog modal-dialog-centered mw-650px">
                <div class="modal-content">
                    <div class="modal-header">
                        <!--begin::Modal title-->
                        <h2>Cập nhật nguồn bán hàng</h2>
...
@section('script')
    <script>
        const routeList = "{{ route('sale_source.list') }}";
        const routeUpdate = "{{ route('sale_source.update') }}";
        filterTable();
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('sale_source.delete') }}");
        }

        function changeStatus(id) {
            $.post("{{ route('sale_source.update') }}", {
                id
            }, function(rs) {
                Toast.fire({
                    icon: rs?.type,
