...
@section('content')
    <div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
        ...
        <div class="content flex-row-fluid" id="kt_content">
            <div class="d-flex justify-content-between py-5">
                <div class="">
                    <h1 class="d-flex text-white fw-bold my-1 fs-3">Quản lý kho hàng</h1>
                ...
                @can('warehouse-create')
                    ...
                    @include('User.warehouse.modal_add')
                @endcan
    ...
    <div class="modal fade" id="modal-edit" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
        <form action="{{ route('warehouse.update') }}" id="form-update" method="POST" enctype="multipart/form-data">
...
@section('script')
    <script>
        const routeList = "{{ route('warehouse.list') }}";
        const routeUpdate = "{{ route('warehouse.update') }}";
        filterTable();
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('warehouse.delete') }}");
        }

        function changeStatus(id) {
            $.post("{{ route('warehouse.update') }}", {
                id
            }, function(rs) {
                Toast.fire({
                    icon: rs?.type,
